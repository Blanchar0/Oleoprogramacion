...
export default function Cycles() {
  ...
  return (
    <div className="space-y-5">
      ...
      {view === 'kanban' && <Kanban cards={filteredCards} />}
...
function Kanban({ cards }: { cards: CycleCard[] }) {
  const columns: CycleStatus[] = ['AL_DIA', 'ALERTA', 'CRITICO', 'SIN_DATOS'];
  return <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">{columns.map((state) => <div key={state} className={`rounded-xl border p-3 ${STATUS_META[state].card}`}><div className="flex items-center justify-between mb-3"><span className="font-bold text-sm text-forest-950">{STATUS_META[state].label}</span><span className="text-xs font-black rounded-full bg-white px-2 py-0.5 border border-gray-200">{cards.filter((card) => card.state === state).length}</span></div><div className="space-y-2 max-h-[520px] overflow-y-auto pr-1">{cards.filter((card) => card.state === state).map((card) => <CycleCardItem key={card.key} card={card} />)}{cards.every((card) => card.state !== state) && <p className="text-xs text-gray-500 p-3 text-center">Sin lotes en este estado.</p>}</div></div>)}</div>;
}
...
function Timeline({ cards, executions, rule, asOf, onLaborChange, rules }: { cards: CycleCard[]; executions: CycleExecution[]; rule: CycleLaborRule; asOf: Date; onLaborChange: (value: string) => void; rules: CycleLaborRule[] }) {
  ...
  return <Card><CardHeader className="gap-3 pb-3 md:flex-row md:items-end md:justify-between"><div><CardTitle className="text-base text-forest-950">Cronograma de {rule.name}</CardTitle><p className="text-xs text-gray-500 mt-1">{rule.scheduleGranularity === 'DIA' ? `Vista diaria de ${formattedMonth}` : `Vista semanal del año ${asOf.getFullYear()}`}. El cero indica una ejecución registrada.</p></div><select value={rule.id} onChange={(event) => onLaborChange(event.target.value)} className="h-9 rounded-lg border border-gray-300 px-3 text-sm text-forest-950 bg-white">{rules.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></CardHeader><CardContent><div className="overflow-auto border rounded-xl"><table className="min-w-max w-full text-xs border-collapse"><thead className="bg-gray-50 sticky top-0 z-10"><tr><th className="sticky left-0 z-20 bg-gray-50 text-left px-3 py-3 border-b min-w-28">Lote</th><th className="sticky left-28 z-20 bg-gray-50 text-left px-3 py-3 border-b min-w-28">Zona</th>{columns.map((date, index) => <th key={date.toISOString()} className="px-1 py-3 border-b text-center text-[10px] font-bold text-gray-500 min-w-8" title={formatDate(toIsoDate(date))}>{rule.scheduleGranularity === 'DIA' ? index + 1 : index + 1}</th>)}<th className="px-3 py-3 border-b text-left min-w-28">Última ejecución</th><th className="px-3 py-3 border-b text-left min-w-24">Estado</th></tr></thead><tbody>{cards.map((card) => <tr key={card.key} className="border-b border-gray-100 hover:bg-gray-50"><td className="sticky left-0 z-10 bg-white px-3 py-2 font-bold text-forest-950">{card.loteCode}</td><td className="sticky left-28 z-10 bg-white px-3 py-2 text-gray-500">{card.zone}</td>{columns.map((date) => { const value = cycleValueAtDate(executions, card.loteCode, rule.id, date, rule); const cellState = getCycleState(value === null ? null : rule.scheduleGranularity === 'DIA' ? value : value * 7, rule); const hasExecution = value === 0; return <td key={date.toISOString()} className="p-0.5 text-center"><span className={`block rounded py-1 text-[10px] font-bold ${hasExecution ? 'bg-forest-700 text-white' : value === null ? 'text-gray-300' : cellState === 'AL_DIA' ? 'bg-emerald-100 text-emerald-800' : cellState === 'ALERTA' ? 'bg-amber-100 text-amber-900' : 'bg-red-100 text-red-800'}`}>{value ?? '—'}</span></td>; })}<td className="px-3 py-2 text-gray-600 whitespace-nowrap">{formatDate(card.lastExecution?.executionDate)}</td><td className="px-3 py-2"><span className={`px-2 py-0.5 rounded-full border text-[10px] font-bold whitespace-nowrap ${stateClass(card.state)}`}>{cycleStateLabel(card.state)}</span></td></tr>)}{cards.length === 0 && <tr><td colSpan={columns.length + 4} className="p-12 text-center text-gray-500">No hay lotes para el filtro seleccionado.</td></tr>}</tbody></table></div></CardContent></Card>;
}
